# Áp dụng thuật toán để tối ưu việc vận chuyển cho các bài toán Logistic

Dự án này áp dụng thuật toán **SHO** (Tối ưu hóa đàn linh cẩu) và các kỹ thuật: **Swap Operation, Swap Sequence, PD_Shift, PD_Exchange, PD_Rearrange**. Những kỹ thuật này cũng được dùng cho các thuật toán **GWO** và **WOA** để đề xuất cách tiếp cận mới, cải thiện lập lộ trình tuyến xe phù hợp với yêu cầu thực tế. ***Mục tiêu là rời rạc hóa các giải thuật meta-heuristic để tìm lời giải tối ưu cho bài toán lập lộ trình tuyến xe trong không gian bài toán lớn.***

---
## Hình ảnh minh họa
[Bổ sung sau]

---

## Hướng dẫn chạy dự án
- **Bước 1**: Lấy dự án về local <br>
`git clone https://github.com/khainam23/logistic.git`

- **Bước 2**: Truy cập vào dự án <br>
`cd logistic`

- **Bước 3**: Checkout qua nhánh công việc (nếu có) hoặc tạo mới dựa trên base của branch **main**
- **Bước 4**: 
  + Mở thư mục bằng các trình IDE
  + Truy cập vào file **pom.xml**
  + Thực hiện **Maven > Install** hoặc các lệnh tương ứng <br>
- ***Lưu ý***: 
  + Nếu trong quá trình thực hiện gặp lỗi thì hãy thực hiện tạo **issue** lấy label **help** để nhận sự hỗ trợ nhanh nhất
  + Chỉ những thành viên được thêm vào dự án mới có quyền pull request và xem xét duyệt
---

## Minh họa cách dùng và ví dụ
[Bổ sung sau]

---

# Thành viên và công việc
- Nguyễn Khải Nam - [các thông tin cá nhân]
- Nguyễn Đô Thanh Phát - [các thông tin cá nhân]

---

## Phần phụ trợ
### Cấu trúc thư mục
- **algorithms/**: Chứa các thuật toán, kỹ thuật xử lý
- **data/**: Chứa các class xử lý dữ liệu hoặc chứa dữ liệu
- **evaluation/**: Dùng cho việc đánh giá, xem kết quả của thuật toán
- **Main.java**: Chạy toàn bộ dự án theo qui trình được thiết lập (nếu có)

---

## Tài liệu tham khảo
[Bổ sung sau]